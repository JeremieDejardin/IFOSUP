# E-Business First class

*Formateur :*
Xavier Jeunejean

## Introduction 
### Qu'est ce que E-Business? 

L'e-business recouvre les applications faisaint aux TIC pour traiter les relations d'une entreprise avec l'ensemble de ses partenaires. 

### Contenu

- Finance
- IT
- Marketing

### Objectif 

Réalisation d'un project e-business commun. 

4 points a couvrir : 

- Besoin client
- Marketing
- IT (Cours de Stéphane)
- Finance 

Le pitch
Presentation succinte du projet : 
- 3-5 ou 7 min pour presenter.

Types : 
Elevator pitch (Very short)
Pitch client
Pitch investor


## TO DO (Slide 12) 
- Business model (Carton de bière)
- Business plan
- Business model canvas
- Pitch (Client ou investor)


## Evaluation

1. Travail quotidien
2. Examen 
   1. Presentation
   2. Site

# Business model

Comment la société fait de l'argent ? 

## SAAS
Abonnement
- Nb abonnés
- Durée moyenne d'abonnement
- Nouveaux abonnés
- Taux de perte d'abonnés
- ... 

## Vente en ligne
Bénéfice par vente
- Nb ventes
- Panier moyen
- Taux de réachat
- Taux de satisfaction
- ...

## Marketplace / Lead gen
% sur mise en relation
- Nb lead/Ventes
- Nb offres
- Equilibre offre/demande
- ... 

## Media 
Publicité
- Nb de page vues
- Connaissance visiteurs (Identification des visiteurs)
- PV des publicités
- ...

# Besoin client
Ecouter les autres, particulièrement ceux d'autres domaines. 

Se demarquer de ses concurrents

... Missing notes...

# Business Plan

## Overview
### 3 interconnected building blocks
Business plan
Marketing
Functionnalities

## Business model



... Missing notes... 

# Glossary 

**Taux de conversion :**
Nombre de visiteur qui devienne client. 
**Click rate :** 
Taux de clique par visite
**Bounce rate :** 
Taux de rebond - celui qui visiteurs qui s'en vont directement
**Conversion rate :**
1. From visitor to prospect
2. From prospect to client
