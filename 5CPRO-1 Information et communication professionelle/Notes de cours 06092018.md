# Communication first class

**Info Formateur :** 
Marie Brasseur
marie.brasseur@ifosupwavre.be

**Contexte :**
Cours partagé avec 2ème compta.

## Introduction
Formateur est expert, pas professeur. 

Format du cours : Exemples et exercices pratiques. 

## 1er cours intro

### Exercice : Brainstorming sur la communication

**Conclusion :** 
Mots clef identifiés :
- outils
- langage
- non-verbal
...Missing notes...

Lors de la mise en commun des mots mis en évidence, le recepteur n'avait pas été mis en avant. 

### Theorie de la comm.

Si 100% represente ce qu'on veut dire 
On estime le rapport suivant : 
- 70% ce qu'on arrive à exprimer
- 60% ce que l'autre entend
- 50% ce que l'autre écoute
- 40% ce que l'autre comprend
- 20% ce que l'autre retient
- 10% ce que l'autre peut restitué/repercuté

### Comprehension vs perception? 

#### Attention
Se mettre en situation de recevoir des informations pour les traiter et prendre une decision

#### Perception 
Une étape essentiel du processus de transformation d'un stimlulus en une representation. Donner une signification a ce qu'on a perçu et y repondre. 

#### Comprehension
Processus psychologique selon lequel on est capable de réflechir et conceptualiser qqchose pour traiter cet objet de la manière la plus appropriée. 


- [ ] Trouver un exemple dans le cadre de travail d'un décalage entre la perception et la comprehension.  
